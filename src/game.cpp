...
//void Game::playerToggleOutfitExtension(uint32_t playerId, int mount, int wings, int aura, int shader, int title)
void Game::playerToggleOutfitExtension(uint32_t playerId, int mount)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
...
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	if (outfit.lookTitle != 0) {
		Title* title = titles.getTitleByID(outfit.lookTitle);
		if (!title) {
		...
		player->setCurrentTitle(title->id);
	}
...
bool Game::reload(ReloadTypes_t reloadType)
{
	switch (reloadType) {
		...
		case RELOAD_TYPE_TITLES: return titles.reload();
