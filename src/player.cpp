...
uint32_t Player::getAttackSpeed() const
{
	...
	if (attackSpeed > 0) {
		return (attackSpeed * weapon->getAttackSpeed()) / 2;
	}
...
void Player::addStorageValue(const uint32_t key, const int32_t value, const bool isLogin/* = false*/)
{
	if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
		if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
			...
			return;
		} else if (IS_IN_KEYRANGE(key, MOUNTS_RANGE) || IS_IN_KEYRANGE(key, WINGS_RANGE) || IS_IN_KEYRANGE(key, AURAS_RANGE) || IS_IN_KEYRANGE(key, SHADERS_RANGE) || IS_IN_KEYRANGE(key, TITLES_RANGE)) {
			// do nothing
		} else {
...
bool Player::hasTitle(const Title* title) const
{
	...
	if (title->premium && !isPremium()) {
		return false;
	}

	const uint8_t tmpTitleId = title->id - 1;

	int32_t value;
	if (!getStorageValue(PSTRG_TITLES_RANGE_START + (tmpTitleId / 31), value)) {
		return false;
	}

	return ((1 << (tmpTitleId % 31)) & value) != 0;
}

void Player::setCurrentTitle(uint16_t titleId)
{
	addStorageValue(PSTRG_TITLES_CURRENTTITLE, titleId);
}

uint16_t Player::getCurrentTitle() const
{
	int32_t value;
	if (getStorageValue(PSTRG_TITLES_CURRENTTITLE, value)) {
		return value;
	}
	return 0;
}
